...
class LookupModule(LookupBase):
    def run(self, terms, variables=None, **kwargs):
        ...
        print("pclass: " + str(get_plugin_class(self)))
        print("all the plugins: " + str(C.config._plugins))

        for term in terms:
            opts = kwargs.copy()
            opts.update(self.parse_term(term))
            print("direct opts: " + str(opts))
            self.set_options(direct=opts)
            # TODO: remove deprecate() if backported fix is available (see method definition)
            deprecate()
            print("process opts: " + str(self._options))
            print("os env val: " + str(os.environ.get('FOODEBUSSY', 'UNSETTTT')))
            print("<<<<<>>>>>>>>>")
            print("os envrion => " + str(os.environ))
    ...
    def set_default_option_env(self, option, var):
        ...
        print("set def opt {}: {}".format(option, var))
        print("set def opt: |{}|".format(self.get_option(option)))
        print("set def opt done")
    ...
    def low_preference_env_vars(self):
        ...
        for opt, envs in LOW_PRECEDENCE_ENV_VAR_OPTIONS.items():
            print("low pref {}: {}".format(opt, envs))
            for env in envs:
                print("low pref set def {}: {}".format(opt, env))
